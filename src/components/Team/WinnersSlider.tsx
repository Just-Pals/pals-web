  ...
  useEffect(() => {
    ...
    const scroll = (currentTime: number) => {
      if (!isPaused && container) {
        ...
        // Smooth scrolling
        container.scrollLeft += scrollSpeed * (deltaTime / 16); // Normalize to 60fps
      ...
      >
        {scrollList.map((w, idx) => (
          <a
            ...
            className="min-w-[220px] xs:min-w-[240px] sm:min-w-[260px] md:min-w-[280px] bg-white rounded-xl sm:rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 cursor-pointer"
          >
            {/* Photo */}
            ...
            {/* Label */}
            <div className="text-center py-3 sm:py-4 text-black px-2">
              <p className="font-bold text-sm sm:text-base mb-1">{w.name}</p>
              <p className="font-medium text-gray-600 text-xs sm:text-sm mb-1">{w.position}</p>
              <p className="font-medium text-gray-500 text-xs">{w.location}</p>
            </div>
          </a>
        ))}
      </div>
